...
@router.post("/")
def createPatientFile(patientFileData:patientFileSchema.PatientFileCreate=Depends(patientFileSchema.patientFileForm),image: UploadFile = File(None),db:Session=Depends(get_db)):
    if image:
