...
def get_review(review_id: int, db: Session = Depends(get_db)):
    review = reviews.get_review_by_id(db, review_id)
    if not review:
...
def list_reviews(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return reviews.get_all_reviews(db, skip, limit)


@router.put("/{review_id}", response_model=ReviewOut)
def update_review(review_id: int, review_data: ReviewUpdate, db: Session = Depends(get_db)):
    updated_review = reviews.update_review(db, review_id, review_data)
...
def delete_review(review_id: int, db: Session = Depends(get_db)):
    deleted = reviews.delete_review(db, review_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="Review not found")
